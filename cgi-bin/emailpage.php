...
//echo "including sendmail.php"; echo phpinfo();
include_once("../php/sendmail.php");

$bDB = false;	// true => the AdminEmail is from DB; 
$bFHeader = true; // true => the "-f $header" as optional header is added to mail() function;
$bHeader = true; // true => using header() function to relay to the next page;
$bAttach = true; // true =>  attach file
$nBodyMsg = 3;	// 0 -  send no email, 1 - send only text mail, 3 - send only html mail;
...
// DEBUG PROPOSE ONLY!!!
$nDebugViewMail = 1	; // 0 - no view, 1 - view only text mail, 2 - view text & html mail, 3 view html mail;
// DEBUG PROPOSE ONLY!!!

$cMailTo = $_POST['mailform_to'];
...
// A T T E N T I O N ! --------------------------------------------------------------------------------------------
$cMailFrom = $_POST['mailform_from'];

$cMailFrom = $_POST['email'] != '' ? $_POST['email'] : $_POST['TAemail'];

$cUrl = $_POST['mailform_url'];
$cSubj = $_POST['mailform_subj'];
	...
	foreach (array("_GET","_POST") as $source) {
		foreach (${$source} as $idx => $value) {
			if(!strpos(' '.$idx,'idemail') === false){
				...
				$nNr += 1;
				$cVal = '$AdminEmail['.($nNr-1).'] = $recget["'.$_POST['idemail'.$nNr].'"];';
				eval($cVal);
			}
		}
	}

	if(!isset($_POST['idemail1'])){
		$AdminEmail[0] = $cMailTo;
	}
//	mysql_close($strlink);	
} else {

  print "3) cMailto = " . $cMailto  . "<br>";
...
$AdminEmail[0]="information@stanfordtravel.com";//"information@stanfordtravel.com";
...
debug_to_console("made it to here.");
...
//
	if($cGCode!=''){
		session_start();
		include_once("../cgi-bin/vImage.php");
		$vImage = new vImage();
		$vImage->loadCodes();
...
//if ($incorrectCode == true)SendContactFormMail();
//SendContactFormMail();
//exit(0);
...
if(isset($_POST[startemail]))
	$startingpoint = "<!-- ".$_POST[startemail]." -->";

if(isset($_POST[stopemail]))
	$endingpoint = "<!-- ".$_POST[stopemail]." -->";
...
$cError='';
//print "Admin Email[0]=" . $AdminEmail[0];
...
if($cMailFrom == '' &&  !isset($_POST['mailform_from']))
...
if($cSubj == '' && !isset($_POST['mailform_subj']))
...
if($cUrl == '' && !isset($_POST['mailform_url']))
...
//
 //debugbreak();
$PHPHtml = str_replace(stripslashes($_POST['mailform_etag']), '', $PHPHtml);
...
//			print $cVar." = ".$_POST["$cVar"]."<br>";
			...
			$cFinal = $cInput." value='".$_POST["$cVar"]."' size=32>";
...
//			print $cVar." = ".$_POST["$cVar"]."<br>";
			...
			$cFinal = $cInput." value='".$_POST["$cVar"]."'>#[".$_POST["$cVar"]."]";
			if(strpos($cInput, "checkbox") === false){
				$cInptVar .= " '".$cVar."' ";
			} else {
				$cCBoxVar .= " '".$cVar."' ";
				if($_POST["$cVar"] == 'Yes')
					$cFinal = $cInput." checked>[".$_POST["$cVar"]."]";
				else
					$cFinal = $cInput.">[No ]";
			}
...
for($nNr = 1; $nNr < $nNrInput+1; $nNr ++){
	...
	if($nName < $nFTag){
		...
		if($nName < $nFTag){
			...
			$cFinal = $cInput.$_POST["$cVar"]."<";
			$PHPHtmlTmp = str_replace(substr($PHPHtmlTmp, $nSTag, $nFTag - $nSTag + 1), "[".$_POST["$cVar"]."]<textarea><", $PHPHtmlTmp);
			$cTxtAVar .= " '".$cVar."' ";
		}
	}
	$nOffset = $nSTag+1;
...
if($bRecDB){
	$_POST['email'] = $_POST['mailform_from'];
	/*
...
?>
	...
	<form name="frm1" action="<?php echo $cUrl?>">
		<input type="submit" value="1">
		<input type="hidden" name="sendok" value="<?php echo $nSend?>">
	</form>
	<form name="frm2" action="<?php echo $refpage?>">
		<input type="submit" value="2">
	</form>
	</div>
	<script type="text/javascript">
	window.location.href="http://<?php echo $cUrl?>";
	</script>
	</body>
	</html>
<?php
...
if ($incorrectCode == false ) {
	$cSubj = "StanfordTravel.com Contact Form";
	$AdminEmail = "information@stanfordtravel.com";
	
	//show all variables posted from form(s)
	//foreach ($_POST as $key => $value)
    //    $postString .= $key.'='.$value.'<br />';
	...
	$body .= "Evening Phone:  " . $_POST['evphone']. "\n";
	$body .= "Fax: " . $_POST['fax']. "\n";
	$body .= "Party: " . $_POST['party']. "\n";
	$body .= "Dates: " . $_POST['dates']. "\n";
	$body .= "Comments:  " . $_POST['comments'] . "\n";
	$body .= $fullLine;
	$body .= "Travel Agency: " . $_POST['TAagency']. "\n";
	$body .= "Name: " . $_POST['TAname']. "\n";
	$body .= "Email: " . $_POST['TAemail']. "\n";
	$body .= "Party: " . $_POST['TAparty']. "\n";
	$body .= "Day Phone: " . $_POST['TAdayphone']. "\n";
	$body .= "Evening Phone: " . $_POST['TAevphone']. "\n";
	$body .= "Fax: " . $_POST['TAfax']. "\n";	
	$body .= "Dates: " . $_POST['TAdates']. "\n";
	$body .= "Comments: " . $_POST['TAcomments']. "\n";	
	$body .= $fullLine;
	$body .= "IP:  '" . getIP() . "'\n";
...
/*
	...
	debug_to_console( "Sending mail...nSend=" . $nSend . "."); 
}*/
...
function get_form_data(){
    ...
    $vars = ($REQUEST_METHOD == 'GET') ? $HTTP_GET_VARS : $_POST;
    foreach ($vars as $k=>$v){
